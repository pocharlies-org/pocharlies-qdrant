...
from supplier_indexer import SupplierIndexer
from pricelist_parser import PricelistParser
from margin_analyzer import MarginAnalyzer
from purchase_recommender import PurchaseRecommender
from supplier_email_monitor import SupplierEmailMonitor
...
supplier_registry: SupplierRegistry = None
supplier_indexer: SupplierIndexer = None
supplier_web_indexer: WebIndexer = None
pricelist_parser: PricelistParser = None
margin_analyzer: MarginAnalyzer = None
purchase_recommender: PurchaseRecommender = None
supplier_email_monitor: SupplierEmailMonitor = None
authenticated_crawler: AuthenticatedCrawler = None
...
async def lifespan(app: FastAPI):
    ...
    # Load shared embedding model for web indexers (BGE-base, English-only)
    # Product/catalog/vault indexers use BGE-M3 (multilingual) via bgem3_encoder module
    ...
    global supplier_registry, supplier_indexer, supplier_web_indexer, pricelist_parser, margin_analyzer, purchase_recommender, supplier_email_monitor, authenticated_crawler
    ...
    try:
        ...
        supplier_registry.seed_if_empty()
        logger.info("Supplier registry initialized")

        supplier_indexer = SupplierIndexer(
            qdrant_url=QDRANT_URL,
            qdrant_api_key=QDRANT_API_KEY,
            model=embedding_model,
            supplier_registry=supplier_registry,
        )
        logger.info("Supplier indexer initialized")

        supplier_web_indexer = WebIndexer(
            qdrant_url=QDRANT_URL,
            qdrant_api_key=QDRANT_API_KEY,
            model=embedding_model,
            collection_name="supplier_pages",
        )
        logger.info("Supplier web indexer initialized")

        pricelist_parser = PricelistParser(
            llm_client=llm_client,
            llm_model=LLM_MODEL,
        )
        logger.info("Pricelist parser initialized")

        margin_analyzer = MarginAnalyzer(
            product_indexer=product_indexer,
            supplier_indexer=supplier_indexer,
            competitor_indexer=competitor_indexer,
            embedding_model=embedding_model,
            qdrant_client=web_indexer.client,
        )
        logger.info("Margin analyzer initialized")

        purchase_recommender = PurchaseRecommender(
            margin_analyzer=margin_analyzer,
            supplier_registry=supplier_registry,
            supplier_indexer=supplier_indexer,
            product_indexer=product_indexer,
            qdrant_client=web_indexer.client,
        ...
        logger.info("Purchase recommender initialized")

        supplier_email_monitor = SupplierEmailMonitor(
            supplier_registry=supplier_registry,
            llm_client=llm_client,
            llm_model=LLM_MODEL,
            redis_client=redis_client,
        )
        logger.info("Supplier email monitor initialized")

        try:
            fc = FirecrawlClient()
            authenticated_crawler = AuthenticatedCrawler(
                firecrawl_client=fc,
                redis_client=redis_client,
            )
            logger.info("Authenticated crawler initialized")
        except Exception as e:
            logger.warning(f"Authenticated crawler init failed: {e}")
    ...
    async def _daily_rebuild():
        ...
        try:
            ...
            # Supplier email intelligence
            if supplier_email_monitor:
                try:
                    results = await supplier_email_monitor.check_emails(days=1)
                    logger.info(f"Supplier email check: {len(results)} intelligence items")
                except Exception as e:
                    logger.warning(f"Supplier email check failed: {e}")
...
5. Be specific — use exact names, prices, and URLs from the context"""
    ...
    if request.use_rag:
        ...
        try:
            if supplier_indexer:
                sup_results = supplier_indexer.search(query=request.query, top_k=5)
                if sup_results:
                    sup_block = "\n".join([
                        f"- [{r.get('name', 'Unknown')}] Supplier: {r.get('supplier_slug', '?')} | "
                        f"Wholesale: {r.get('wholesale_price', 'N/A')} {r.get('currency', 'EUR')} | "
                        f"RRP: {r.get('rrp', 'N/A')} | URL: {r.get('url', '')}"
                        for r in sup_results
                    ])
                    context_sections.append(f"## Supplier Product Results\n{sup_block}")
        except Exception as e:
            logger.warning(f"Supplier search failed in chat: {e}")
...
class SupplierCrawlRequest(BaseModel):
    url: str
    supplier_slug: str
    max_pages: int = 50
    max_depth: int = 2
...
class SupplierAuthCrawlRequest(BaseModel):
    supplier_slug: str
...
class SupplierProductSearchRequest(BaseModel):
    query: str
...
@app.post("/supplier/registry")
async def supplier_create(request: SupplierCreateRequest):
    """Register a new supplier."""
    if not supplier_registry:
        raise HTTPException(status_code=503, detail="Supplier registry not initialized")
    data = request.model_dump()
    result = supplier_registry.upsert_supplier(data)
    return result


@app.get("/supplier/registry")
async def supplier_list(status: Optional[str] = None, type: Optional[str] = None):
    """List all registered suppliers."""
    if not supplier_registry:
        raise HTTPException(status_code=503, detail="Supplier registry not initialized")
    return {"suppliers": supplier_registry.list_suppliers(status=status, type_filter=type)}


@app.get("/supplier/registry/{slug}")
async def supplier_get(slug: str):
    """Get a single supplier by slug."""
    if not supplier_registry:
        raise HTTPException(status_code=503, detail="Supplier registry not initialized")
    s = supplier_registry.get_supplier(slug)
    if not s:
        raise HTTPException(status_code=404, detail=f"Supplier not found: {slug}")
    return s


@app.delete("/supplier/registry/{slug}")
async def supplier_delete(slug: str):
    """Delete a supplier from the registry."""
    if not supplier_registry:
        raise HTTPException(status_code=503, detail="Supplier registry not initialized")
    ok = supplier_registry.delete_supplier(slug)
    if not ok:
        raise HTTPException(status_code=404, detail=f"Supplier not found: {slug}")
...
async def supplier_search(q: str, top_k: int = 5):
    """Semantic search across registered suppliers."""
    if not supplier_registry:
        raise HTTPException(status_code=503, detail="Supplier registry not initialized")
    return {"results": supplier_registry.search(q, top_k=top_k)}


@app.get("/supplier/stats")
async def supplier_stats():
    """Get supplier registry statistics."""
    if not supplier_registry:
        raise HTTPException(status_code=503, detail="Supplier registry not initialized")
    stats = supplier_registry.get_stats()
    if supplier_indexer:
        stats["product_stats"] = supplier_indexer.get_stats()
    return stats


@app.post("/supplier/crawl")
async def supplier_crawl(request: SupplierCrawlRequest):
    """Crawl a supplier website and index products."""
    if not supplier_indexer or not supplier_web_indexer:
        raise HTTPException(status_code=503, detail="Supplier indexer not initialized")
    if not validators.url(request.url):
        raise HTTPException(status_code=400, detail=f"Invalid URL: {request.url}")

    # Verify supplier exists
    sup = supplier_registry.get_supplier(request.supplier_slug) if supplier_registry else None
    if not sup:
        raise HTTPException(status_code=404, detail=f"Supplier not found: {request.supplier_slug}")
    ...
    crawl_jobs[job_id] = placeholder_job
    crawl_queue.append(job_id)
    placeholder_job._supplier_slug = request.supplier_slug
    placeholder_job._supplier_indexer = True

    asyncio.create_task(_process_crawl_queue())

    return {"job_id": job_id, "status": "queued", "url": request.url, "supplier": request.supplier_slug}


@app.post("/supplier/crawl-authenticated")
async def supplier_crawl_authenticated(request: SupplierAuthCrawlRequest):
    """Crawl a supplier's B2B portal with authentication."""
    if not authenticated_crawler:
        raise HTTPException(status_code=503, detail="Authenticated crawler not initialized")
    if not supplier_registry:
        raise HTTPException(status_code=503, detail="Supplier registry not initialized")

    sup = supplier_registry.get_supplier(request.supplier_slug)
    if not sup:
        raise HTTPException(status_code=404, detail=f"Supplier not found: {request.supplier_slug}")

    start_url = request.start_url or sup.get("b2b_portal") or sup.get("website")
    if not start_url:
        raise HTTPException(status_code=400, detail="No URL available for this supplier. Provide start_url or set website/b2b_portal.")

    try:
        pages, page_count = await authenticated_crawler.authenticated_crawl(
            supplier_slug=request.supplier_slug,
            start_url=start_url,
            max_pages=request.max_pages,
        ...
        # Index crawled products
        if pages:
            count = supplier_indexer.index_from_pages(
                pages=pages,
                supplier_slug=request.supplier_slug,
                is_b2b=True,
            )
            supplier_registry.update_products_indexed(request.supplier_slug, count)
            supplier_registry.update_last_crawled(request.supplier_slug)
            

        return {"status": "ok", "supplier_slug": request.supplier_slug, "pages_crawled": page_count, "products_indexed": count if pages else 0}
    except Exception as e:
        logger.error(f"Authenticated crawl failed for {request.supplier_slug}: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/supplier/products/search")
async def supplier_product_search(request: SupplierProductSearchRequest):
    """Search supplier products."""
    if not supplier_indexer:
        raise HTTPException(status_code=503, detail="Supplier indexer not initialized")
    results = supplier_indexer.search(
        query=request.query,
        supplier_slug=request.supplier_slug,
        top_k=request.top_k,
...
@app.post("/supplier/match/{supplier_slug}")
async def supplier_match_catalog(supplier_slug: str):
    """Match supplier products to our catalog."""
    if not supplier_indexer:
        raise HTTPException(status_code=503, detail="Supplier indexer not initialized")
    matched = supplier_indexer.match_to_our_catalog(supplier_slug)
    return {"supplier": supplier_slug, "matched": matched}


@app.get("/supplier/products/stats")
async def supplier_product_stats():
    """Get supplier product index statistics."""
    if not supplier_indexer:
        raise HTTPException(status_code=503, detail="Supplier indexer not initialized")
    return supplier_indexer.get_stats()


@app.delete("/supplier/products/{supplier_slug}")
async def supplier_product_delete(supplier_slug: str):
    """Delete all indexed products for a supplier."""
    if not supplier_indexer:
        raise HTTPException(status_code=503, detail="Supplier indexer not initialized")
    ok = supplier_indexer.delete_supplier_products(supplier_slug)
    if not ok:
        raise HTTPException(status_code=500, detail=f"Failed to delete products for {supplier_slug}")
    return {"status": "deleted", "supplier": supplier_slug}


@app.post("/supplier/upload-pricelist")
async def supplier_upload_pricelist(request: PricelistUploadRequest):
    """Parse and index a supplier pricelist."""
    if not pricelist_parser or not supplier_indexer:
        raise HTTPException(status_code=503, detail="Pricelist parser not initialized")
    ...
    try:
        products = pricelist_parser.parse(
            content=request.content,
            format=request.format,
            currency=request.currency,
        )
        count = supplier_indexer.index_products(
            products=products,
            supplier_slug=request.supplier_slug,
            source_type="pricelist",
        )
        supplier_registry.update_products_indexed(request.supplier_slug, count)
        supplier_registry.update_last_pricelist(request.supplier_slug)
        return {"supplier": request.supplier_slug, "products_parsed": len(products), "products_indexed": count}
    except Exception as e:
        logger.error(f"Pricelist upload failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/supplier/margins")
async def supplier_margins(handle: Optional[str] = None, brand: Optional[str] = None, limit: int = 50):
    """Analyze margins for a product or brand."""
    if not margin_analyzer:
        raise HTTPException(status_code=503, detail="Margin analyzer not initialized")

    if handle:
        report = margin_analyzer.analyze_product(handle)
        if not report:
            raise HTTPException(status_code=404, detail=f"Product not found: {handle}")
        return report.to_dict()
    elif brand:
        return margin_analyzer.analyze_brand(brand, limit=limit)
    else:
        raise HTTPException(status_code=400, detail="Provide handle or brand parameter")


@app.get("/supplier/compare")
async def supplier_compare(q: str, top_k: int = 10):
    """Compare supplier prices for a query."""
    if not margin_analyzer:
        raise HTTPException(status_code=503, detail="Margin analyzer not initialized")
    return {"comparisons": margin_analyzer.compare_suppliers(q, top_k=top_k)}


@app.get("/supplier/margin-alerts")
async def supplier_margin_alerts(limit: int = 100):
    """Get margin alerts (negative margins, thin margins, etc.)."""
    if not margin_analyzer:
        raise HTTPException(status_code=503, detail="Margin analyzer not initialized")
    return {"alerts": margin_analyzer.flag_issues(limit=limit)}


@app.get("/supplier/recommendations")
async def supplier_recommendations(limit: int = 50):
    """Get restock recommendations."""
    if not purchase_recommender:
        raise HTTPException(status_code=503, detail="Purchase recommender not initialized")
    return {"recommendations": [r.to_dict() if hasattr(r, 'to_dict') else r for r in purchase_recommender.restock_recommendations(limit=limit)]}


@app.get("/supplier/recommendations/gaps")
async def supplier_gaps(limit: int = 50):
    """Find products competitors sell that we don't stock."""
    if not purchase_recommender:
        raise HTTPException(status_code=503, detail="Purchase recommender not initialized")
    return {"gaps": purchase_recommender.gap_analysis(limit=limit)}


@app.get("/supplier/recommendations/delist")
async def supplier_delist(limit: int = 50):
    """Find products to consider delisting."""
    if not purchase_recommender:
        raise HTTPException(status_code=503, detail="Purchase recommender not initialized")
    return {"candidates": purchase_recommender.delist_candidates(limit=limit)}


@app.post("/supplier/recommendations/purchase-order")
async def supplier_purchase_order(request: PurchaseOrderRequest):
    """Generate a purchase order for a supplier."""
    if not purchase_recommender:
        raise HTTPException(status_code=503, detail="Purchase recommender not initialized")
    return purchase_recommender.generate_purchase_order(
        supplier_slug=request.supplier_slug,
        product_handles=request.product_handles,
    )


@app.post("/supplier/email-check")
async def supplier_email_check(days: int = 7):
    """Check supplier emails for intelligence."""
    if not supplier_email_monitor:
        raise HTTPException(status_code=503, detail="Supplier email monitor not initialized")
    results = await supplier_email_monitor.check_emails(days=days)
    return {"results": results, "count": len(results)}


@app.get("/supplier/email-digest")
async def supplier_email_digest(supplier: Optional[str] = None, days: int = 7):
    """Get supplier email intelligence digest."""
    if not supplier_email_monitor:
        raise HTTPException(status_code=503, detail="Supplier email monitor not initialized")
    return await supplier_email_monitor.get_digest(supplier_slug=supplier, days=days)
